...
@section('content')
    <div class="mb-4">
        <h2 class="mb-0">Edit Produk: {{ $produk->nama_produk }}</h2>
        <p class="mb-0 text-secondary">Isi informasi produk baru</p>
    </div>

    <form action="{{ route('user.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" id="editForm">
        @csrf
        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control @error('varian') is-invalid @enderror"
                                    id="varian" name="varian" value="{{ old('varian', $produk->varian) }}">
                                @error('varian')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="card border mb-3">
                    ...
                    <div class="card-body">
                        <div class="image-upload-container" id="imageUploadContainer">
                            <div class="upload-area" id="uploadArea" style="{{ $produk->foto ? 'display: none;' : '' }}">
                            ...
                            <div class="preview-area" id="previewArea"
                                style="{{ $produk->foto ? '' : 'display: none;' }}">
                                <img id="imagePreview" src="{{ $produk->foto ? asset('storage/' . $produk->foto) : '' }}"
                                    alt="{{ $produk->foto ? $produk->nama_produk : 'Preview' }}" class="preview-image">
                        ...
                        <small class="text-muted">Format: JPG, JPEG, PNG. Maksimal 5MB. Kosongkan jika tidak ingin mengubah
                            foto.</small>
                        @error('foto')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-column flex-md-row gap-2">
                            ...
                            <button type="submit" class="btn btn-primary w-100" id="submitBtn">
                                <span id="submitText">
                                    <i class="ai-check"></i> Update Produk
                                </span>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin menghapus produk <strong>{{ $produk->nama_produk }}</strong>?
                    </p>
                    <p class="text-danger mb-0"><small>Tindakan ini tidak dapat dibatalkan.</small></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light border" data-bs-dismiss="modal">Batal</button>
                    <form action="{{ route('user.produk.destroy', $produk->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            removeBtn.addEventListener('click', function() {
                ...
                fileInput.parentNode.appendChild(removeImageInput);
            });
